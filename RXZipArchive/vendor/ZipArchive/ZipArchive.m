...
#define RXLog(__format,  ...) printfLog([NSString stringWithFormat:@"%s", __FUNCTION__], __LINE__, [NSString stringWithFormat:(__format), ##__VA_ARGS__])
...
#pragma mark - ~~~~~~~~~~~ 打印日志 ~~~~~~~~~~~~~~~
void printfLog(NSString * message, int line, NSString * method) {
#if DEBUG
    NSLog(@"\n method=%@, line = %zd\n %@ \n\n", method, line, message);
#endif
}
      ...
      completionHandler:(void (^)(NSString *path, BOOL succeeded, NSError * __nullable error))completionHandler
{
    ...
    if (unzGoToFirstFile(zip) != UNZ_OK)
        ...
        RXLog(@"unzGoToFirstFile(zip) != UNZ_OK");
        return NO;
        }
        ...
        @autoreleasepool {
            ...
            if ([delegate respondsToSelector:@selector(zipArchiveShouldUnzipFileAtIndex:totalFiles:archivePath:fileInfo:)]) {
                if (![delegate zipArchiveShouldUnzipFileAtIndex:currentFileNumber
                    ...
                    success = NO;
                    canceled = YES;
                    RXLog(@"delegate respondsToSelector:@selector(zipArchiveShouldUnzipFileAtIndex:totalFiles:archivePath:fileInfo:");
                    break;
                }
            }
            ...
            if (nil != err) {
                ...
                RXLog(@"[SSZipArchive] Error: %@", err.localizedDescription);
                RXLog(@"nil != err");
            }
            ...
            if (!fileIsSymbolicLink) {
                ...
                if (readBytes >= 0) {
                    ...
                    if (fp) {
                        if ([[[fullPath pathExtension] lowercaseString] isEqualToString:@"zip"]) {
                            RXLog(@"Unzipping nested .zip file:  %@", [fullPath lastPathComponent]);
                        ...
                        if (preserveAttributes) {
                            ...
                            if (fileInfo.dosDate != 0) {
                                ...
                                if (attr) {
                                    if ([fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO) {
                                        ...
                                        RXLog(@"[SSZipArchive] Failed to set attributes - whilst setting modification date");
                                        RXLog(@"fileManager setAttributes:attr ofItemAtPath:fullPath error:nil] == NO");
                                        RXLog(@"[SSZipArchive] Failed to set attributes - whilst setting modification date");
                                    }
                                }
                            }
                            ...
                            if (permissions != 0) {
                                ...
                                if ([fileManager setAttributes:attrs ofItemAtPath:fullPath error:nil] == NO) {
                                    ...
                                    RXLog(@"[SSZipArchive] Failed to set attributes - whilst setting permissions");
                                    RXLog(@"[SSZipArchive] Failed to set attributes - whilst setting permissions");
                                }
                ...
                if (symlinkError != 0)
                    {
                    RXLog(@"Failed to create symbolic link at \"%@\" to \"%@\". symlink() error code: %d", fullPath, destinationPath, errno);
                    RXLog(@"Failed to create symbolic link at \"%@\" to \"%@\". symlink() error code: %d", fullPath, destinationPath, errno);
                    }
                }
    ...
    if (success && preserveAttributes) {
        ...
        for (NSDictionary * d in directoriesModificationDates) {
            if (![[NSFileManager defaultManager] setAttributes:@{NSFileModificationDate: d[@"modDate"]} ofItemAtPath:d[@"path"] error:&err]) {
                RXLog(@"[SSZipArchive] Set attributes failed for directory: %@.", d[@"path"]);
                
            }
            if (err) {
                RXLog(@"[SSZipArchive] Error setting directory file modification date attribute: %@",err.localizedDescription);
            }
        }
#if !__has_feature(objc_arc)
